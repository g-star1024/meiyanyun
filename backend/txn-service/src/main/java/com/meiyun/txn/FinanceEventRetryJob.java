 ...
 */
@Component
public class FinanceEventRetryJob {

    private static final Logger log = LoggerFactory.getLogger(FinanceEventRetryJob.class);
    ...
    public FinanceEventRetryJob(FinanceEventRepository eventRepo, RestTemplate restTemplate) {
    ...
    public void dispatchPending() {
        List<FinanceEvent> pending = eventRepo.findFirst50ByStatusOrderByEventIdAsc("PENDING");
        if (pending.isEmpty()) return;
        for (FinanceEvent e : pending) {
            try {
                dispatchOne(e);
            } catch (Exception ex) {
                // 单条异常不影响同批其余事件
                log.warn("资金事件投递兜底异常 eventId={} bizRef={}: {}",
     ...
     */
    private void dispatchOne(FinanceEvent e) {
        ...
        try {
            ...
            e.setLastError(null);
            eventRepo.save(e);
            log.info("资金事件投递成功 eventId={} type={} bizRef={} 重试{}次",
            ...
            if (e.getRetryCount() >= MAX_RETRY) {
                ...
                eventRepo.save(e);
                log.warn("资金事件投递失败，待重试 eventId={} bizRef={} 第{}次: {}",
                        e.getEventId(), e.getBizRef(), e.getRetryCount(), ex.getMessage());
            }
        }
    }

    private void markDead(FinanceEvent e, String reason) {
        e.setStatus("DEAD");
        e.setLastError(clip(reason));
        eventRepo.save(e);
        log.error("资金事件置 DEAD（需财务对账人工介入）eventId={} type={} bizRef={} reason={}",
