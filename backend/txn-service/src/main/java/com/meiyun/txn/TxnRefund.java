...
public class TxnRefund {
    ...
    // B19：L3 大额退款三签（REVIEW 店长 → REGION 区域经理 → FINANCE 财务）
    @Column(length = 32)
    ...
    // B95 合同联动：挂关联合同号（可空；空＝旧链零行为变化）
    @Column(name = "contract_no", length = 24)
    private String contractNo;

    // B95 合同违约金（期后从应退额扣留；冷静期内 0；语义＝扣留非额外收费）
    @Column(name = "penalty_amt", nullable = false)
    private Long penaltyAmt = 0L;
