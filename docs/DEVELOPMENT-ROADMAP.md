...
| ⑤ | 营销与留存 | 12 | 0 | 6 | **营销 9 页收口✅**（B22）+ **赠金高级规则✅/外部渠道回传✅（B26）**+ **充值赠金✅（B35 收银台抵扣闭合，营销域 🔧 清零）**；落地页/转介绍 Backlog |
...
| 营销设置 | 🔧 | B34 | 前后端均实 | 不在 B22 9 页内；**B34 纵深**：新增「券核销兜底门店」配置项(marketing_cfg.writeoff_fallback_store_code，写入前门店存在性校验中文 400、空白归一 null、同值不写冗余审计)，前端 /m5-settings 审批流分组真实门店下拉；仍随营销规则引擎(赠金高级规则/频控配置化)整体收口，另 GET/POST 字段名不对称(weeklyPushLimit vs weeklyLimit)留 Backlog |
| 充值赠金 | ✅ | B18/B26/B35 | DELIVERY-P5-B35，GrantService/InternalGrantController/PaymentService/OrderView | B18 账本+发赠金、B26 满赠阶梯/有效期/报表；**B35 收口**：grant_deduction 扣减流水+FIFO 带锁扣减/幂等/超额 422/GRANT_DEDUCT 审计，marketing internal 端点（X-Internal-Token），txn 转发查余额（cashier:view，散客/不可用降级 available:false）+grant 支付（散客 400/同单第二笔 409），收银台「营销赠金」磁贴与混合支付（template/style 零改动）；消费满额自动发赠金触发（issueByRule 现仅手工端点）留 Backlog |
...
| 消费满额自动发赠金触发（issueByRule 接消费事件） | B35 新增 | `GrantService.issueByRule()` 已就绪但当前**无任何消费事件调用方**，发赠金仅走手工 POST /grants/issue 端点；CONSUME_THRESHOLD 规则不会因订单收款自动触发发放。需在收款完成（FUND_ENTRY 之后）事件链路上挂规则匹配+幂等发放，注意与 B25 自动积分（AUTOPOINTS）复用同一事件源、防同一订单重复发赠 | 营销收口批（B37 规则引擎同期） |
...
| marketing_cfg GET/POST 字段名不对称 | B34 新增 | 读接口返回 `weeklyPushLimit`，写接口收 `weeklyLimit`（另有数个同类），前端 store 靠六处手工映射兜住；需定 DTO 统一命名并同步前端，属破坏性改动应独立成卡 | 近线技术债 |
| marketing_cfg 无 Flyway 迁移脚本 | B34 新增 | 全表 15 列（含 B34 新增 `writeoff_fallback_store_code`）均靠 `ddl-auto=update` 生成，生产环境无可回溯的版本化 DDL；建议随「Flyway 基线补建」统一批处理 | 中期治理 |
