...
| ⑤ | 营销与留存 | 12 | 0 | 6 | **营销 9 页收口✅**（B22）+ **赠金高级规则✅/外部渠道回传✅（B26）**+ **充值赠金✅（B35 收银台抵扣闭合，营销域 🔧 清零）**+ **营销设置✅（B37 卡1 DTO 契约收口）+ 消费满额自动发赠金✅（B37 卡2 轮询 Job）**；落地页/转介绍/退款回加 Backlog |
...
| 退款回加赠金（消费满额规则的逆向链路） | B37 新增 | B37 卡2 只做「收款满额→发赠」正向链路；订单退款后是否回加/扣回已发赠金未实现——需先定义业务语义：赠金已被抵扣（grant_deduction 有流水）时如何追回、已过期时如何处理、部分退款按门槛差额还是全额回加；建议与退款审批链路专项一并设计，不在规则引擎批内夹带 | 交易-营销联动专项 |
...
| marketing_cfg 等营销/审计表无 Flyway 迁移脚本 | B34 新增 | marketing_cfg 全表 15 列（含 B34 `writeoff_fallback_store_code`）、**B37 卡2 新增 auto_grant_state 整表**均靠 `ddl-auto=update` 生成；audit-service 同样无 Flyway——**B37 卡2 实证 Hibernate update 不会加宽已有列**：`audit_log.txn_no` 经现网手动 `ALTER TABLE ... TYPE VARCHAR(128)` 才修复（实体 length 24→128 + 基线 db/schema.sql 同步，该基线脚本在 gitignore 的 db/ 下未入库）。生产环境无可回溯版本化 DDL，建议随「audit-service 引入 Flyway + 全量基线补建」统一批处理 | 中期治理 |
| audit_log 基线 schema 与实体/现网的历史漂移 | B37 新增 | B37 修 txn_no 列宽时发现：基线 db/schema.sql 中 audit_log.biz_type 仍为 VARCHAR(16)，而实体与现网（B32 ALTER）已是 VARCHAR(32)；本次仅同步 txn_no 一处，biz_type 漂移留待 Flyway 基线批一并校准（db/ 目录被 .gitignore 忽略，基线脚本本身未版本化也是成因之一） | 中期治理（随 Flyway 基线） |
| 存量脏数据核销流水/营销资产 SST 门店回填（**B36 实测更正**） | B34 新增/B36 更正 | B34 登记「WR20260908-000004~000007 共 4 条」经 B36 库内实测**更正为 7 条：WR20260908-000001~000007**，store_code 均为 core 库不存在的 SST01（B34 之前硬编码所致，门控自 B34/B36 起只防新增）；另补登同源存量：**WR-SEED 前缀 4 条（9/1-9/3）**、`marketing_asset` SPECIFIED+SST01 **2 条**。是否回填为真实门店、回填成哪家，需业务确认后再动，不擅自改历史业务数据（append-only） | 待业务确认 |
