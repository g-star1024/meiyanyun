...
> **最近更新**：2026-09-09（B25 交付：域①客户域自动化三件套——标签自动化规则引擎（TA###/TagAutoRuleJob/5 类条件）、消费自动积分（txn 内部只读端点拉取已收款/已退款订单 + AUTOPOINTS 幂等发分/回退）、等级定时批处理（LevelMonthlyJob 每月1号 cron + autoUpgrade 开关收敛）；整体进度 54→57/166=34%）。B24 基线见上。
