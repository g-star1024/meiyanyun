...
>
> **基线**：P5-B19 已交付（commit `fbe65c2`，2026-09-08）。
> **最近更新**：2026-09-08（B20 启动，建立本台账）。
...
|---|---|---|---|
| ✅ 已完成 | **32** | 19% | 三者齐备，真实闭环 |
| 🔧 半成品 | **27** | 16% | 后端有端点/前后端均实但缺验证或前端 mock |
...
| 耗材库存 | ✅ | B8/B13 | 库存真实 | — |
| 组织树 | 🔧 | — | 部分真实 | 待接线收口 |
| 消息通知中心 | ⬜ | — | 前端 mock，**后端无端点** | B20 催办需落点，近线 |
| 交接班 | ⬜ | — | — | 近线缺口 |
...
| **P5-B20** | **2026-09-08 进行中** | **审批 SLA 收口 + 退卡手续费落账 + 候选预过滤 + sign_tier 回溯** | — | — | 审批中心、退卡、消息通知（催办落点） |
...
### B20 本批拟收口（不入 Backlog，仅登记范围）

| 事项 | 来源 | 做法 |
|---|---|---|
| L3 审批 SLA 超时扫描 + 催办 | B19 移交 | txn 定时任务（复用 @Scheduled 范式），阶段截止/超时标记/催办；催办通知落点见下 |
| 审批中心复核人/指派人候选预过滤 | B19 移交 | 复用 B19 listStaff({storeCode,roleCode,region}) 兼岗并集，前端选择器过滤 |
| sign_tier 历史数据回溯 | B19 移交（A4） | seed/prod PG 一次性修正历史单 sign_tier |
| 退卡手续费 fund_entry 收入分录 | B18 移交 | 终审 confirmRefund 事件→finance postEntries（idemKey 幂等，FEE/收入科目） |
...
| 退卡冻结 SLA 超时提醒 | B18 移交 | 消息通知中心端点（B20 催办落点可复用） | B21 |
| 赠金高级规则（满赠阶梯/有效期/报表） | B18 移交 | 营销规则引擎 | 营销收口批 |
| 86 张卡开卡首笔流水修复 | B17 移交 | 售卡开卡链路排查 | B21（数据修复） |
| 去划扣项目智能预填 | B17 移交 | 卡项-项目映射 | 交易收口批 |
| 消息通知中心后端端点 | 全局缺口 | 后端现无通知端点，/notifications 为 mock；B20 催办需最小落点 | **B20 最小落地 / 完整中心 B21** |
