...
> **最近更新**：2026-09-09（B25 交付：域①客户域自动化三件套——标签自动化规则引擎（TA###/TagAutoRuleJob/5 类条件）、消费自动积分（txn 内部只读端点拉取已收款/已退款订单 + AUTOPOINTS 幂等发分/回退）、等级定时批处理（LevelMonthlyJob 每月1号 cron + autoUpgrade 开关收敛）；整体进度 54→57/166=34%。同日补运行态闭环：docker 双栈（meiyun_core + meiyun_seed）Flyway 建表 + 打撤标/发分幂等/退款回退/等级只升不降五项冒烟全部取证，过程中修复 3 个真实运行态缺陷、冒烟改库已全量还原，详见 DELIVERY-P5-B25 §4）。B24 基线见上。
...
| ⬜ 未开始 | **102** | 62% | mock/占位，含大量远期独立阶段页面 |
| **合计** | **166** | 100% | |
...
| P5-B25 | 2026-09-09 | 域①客户域自动化三件套：标签自动化规则引擎（TA### 规则 CRUD + TagAutoRuleJob 定时扫描 + 5 类条件 + 打/撤标 + 审计）、消费自动积分（txn 内部只读端点拉取已收款/已退款订单 + 积分规则折算 + AUTOPOINTS 幂等键发分/回退）、等级定时批处理（LevelMonthlyJob 每月1号 cron + autoUpgrade 开关收敛 + 汇总审计）。**运行态闭环补丁**：迁移避让全库共享版本号 V6→V9；双栈冒烟修复 3 个真实缺陷（退款投影误用 txn_refund.customer 姓名列、scan 大事务 UnexpectedRollbackException 毒化整批、重放无视 AdjustResult.created() 虚增计数/重复审计）；cron 外部化 | `00a06e3`（首版代码）、运行态补丁与双提交见 DELIVERY-P5-B25 §六 | DELIVERY-P5-B25 | 标签自动化规则、消费自动积分、等级定时批处理 3 模块 ✅（54→57/166=34%；客户域 ✅5→8、⬜13→10；docker 双栈五项冒烟取证 + 冒烟改库全量还原） |
