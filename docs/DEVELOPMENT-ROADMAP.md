...
| txn-service 扣减侧单测零覆盖 | B35 新增 | B35 的 grant 支付/MarketingGrantClient/FIFO 联动仅靠 marketing 域 84 单测 + 网关 curl + 浏览器验证，txn-service 本身无单测工程（全服务历史欠账）；grant 散客 400/第二笔 409/降级 available:false/幂等重放分支应有服务内单测防回归 | 近线技术债（随 txn 测试基建批） |
...
| SST01 幽灵门店码残留清理批 | B34 新增 | B34 已消灭核销主链路的硬编码 SST01（core 库无此门店），但另有 5 处残留：`frontend/src/utils/storeContext.ts:23` `DEFAULT_STORE_CODE='SST01'` 兜底常量（含 L8/L59 已失实注释）、`CouponWriteoffDataInitializer:103-104` 与 `db/01_master.sql:48` 店名冲突、`MarketingAssetDataInitializer:47,63`、`m5Core.ts:180,254` mock 值；需统一改读真实门店表或删除 | 近线技术债 |
...
| 历史脏数据 WR20260908-000004~000007 门店回填 | B34 新增 | 4 条核销流水 store_code 为不存在的 SST01（B34 之前的硬编码所致）；是否回填为真实门店、回填成哪家，需业务确认后再动，不擅自改历史业务数据 | 待业务确认 |
| StoreNameResolver 不可达与不存在不可区分 | B34 新增 | store-service 超时/宕机与门店编码确实不存在，当前都回落到「原样返回编码」，页面无法区分是脏数据还是链路故障；建议分层返回并在 UI 上区别提示 | 近线健壮性 |
| `/internal/first` 首家门店排序口径未文档化 | B34 新增 | 三级兜底链末环依赖该端点，但「首家」按什么排序（id/编码/创建时间）未在接口契约中写明，换实现即静默改变兜底结果；需补契约说明并加断言测试 | 近线契约 |
