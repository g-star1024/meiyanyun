...
>
> **基线**：P5-B23 已交付（2026-09-09，客户域 4 卡全收口：积分商城、积分账户/人工调分、客户标签、会员等级）。
> **最近更新**：2026-09-09（B23 交付：客户域积分商城/积分账户/客户标签/会员等级全部从 mock/半成切为真实闭环，整体进度 43→47/166=28%）。
...
|---|---|---|---|
| ✅ 已完成 | **47** | 28% | 三者齐备，真实闭环 |
| 🔧 半成品 | **12** | 7% | 后端有端点/前后端均实但缺验证或前端 mock |
| ⬜ 未开始 | **107** | 65% | mock/占位，含大量远期独立阶段页面 |
| **合计** | **166** | 100% | |

> 注：P5-B23 客户域 4 卡收口（积分商城/积分账户人工调分/客户标签/会员等级）经网关、PG 对账与真实浏览器验证闭合，页面口径 43→**47/166=28%**（域① 客户与会员 ✅ 1→5、🔧 4→0）；商城双签审核/履约幂等、人工调分 clientToken 幂等、标签防重打标、等级实时人数与自动升级均真实落地。标签自动化、消费自动积分、等级定时批处理与自动降级引擎列 Backlog。P5-B22 营销 9 页收口见批次时间线；外部广告渠道与归因模型仍为外部平台依赖。

### 两个口径，避免「完成度错觉」

- **全量页面口径：28%**——分母含 M1 集团 / M2 门店运营 / M3 客户运营 / A1 AI 中心 / T2 数据 / T4 算力 / C 端移动端等**远期独立阶段**约 **81 个页面**（占 ⬜ 的大头）。这些是与当前门店中台主线并行的后续大阶段，不应算作「主线欠债」。
- **门店中台核心主线口径（交易 + 财务 + 组织权限 + 门店主数据 + 营销 + 客户运营基础）**：**核心闭环已打通** ✅。退款/退卡/划扣/审批/卡项/资金/对账/月结/成本/提成/权限/主数据链路全部真实落地，**营销 9 页与客户域 4 卡均收口闭合**（券/推送/核销/ROI、积分商城/积分账户/标签/等级真实闭环）；当前工作是 **🔧 接线收口**（财务边角页、营销设置、工作台/EMR 会话区）与**近线 ⬜ 补点**（消息通知多渠道、排队接待、资产转移等）。
