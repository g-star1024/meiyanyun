...
>
> **基线**：P5-B21 已交付（2026-09-08，审批转交/加签目标人硬校验 + 通知偏好持久化 + SLA 催办免打扰）。
> **最近更新**：2026-09-08（B21 交付：审批指派人/会签硬校验、notify_preference 偏好表+GET/PUT 端点、SLA 催办偏好免打扰双向实证、退卡冻结 SLA 覆盖勾销）。
...
|---|---|---|---|
| ✅ 已完成 | **43** | 26% | 三者齐备，真实闭环 |
| 🔧 半成品 | **16** | 10% | 后端有端点/前后端均实但缺验证或前端 mock |
| ⬜ 未开始 | **107** | 65% | mock/占位，含大量远期独立阶段页面 |
| **合计** | **166** | 100% | |

> 注：P5-B22 营销 9 页收口（优惠券/精准推送/券核销/营销总览/素材库/海报/直播短视频/营销ROI/渠道业绩）逐页网关 + 浏览器验证闭合，页面口径 34→**43/166=26%**（域⑤ 营销 ✅ 0→9）；push 写链路补幂等/审计/周频控/违禁词四件套，券核销门店三级兜底，海报推荐人接真实员工。外部广告渠道（美团/抖音/小红书/大众点评/新氧）与归因模型为外部平台依赖，列 Backlog。P5-B20/B21 为既有 ✅ 模块纵深增强（不新增页面）。

### 两个口径，避免「完成度错觉」

- **全量页面口径：26%**——分母含 M1 集团 / M2 门店运营 / M3 客户运营 / A1 AI 中心 / T2 数据 / T4 算力 / C 端移动端等**远期独立阶段**约 **81 个页面**（占 ⬜ 的大头）。这些是与当前门店中台主线并行的后续大阶段，不应算作「主线欠债」。
- **门店中台核心主线口径（交易 + 财务 + 组织权限 + 门店主数据 + 营销）**：**核心闭环已打通** ✅。退款/退卡/划扣/审批/卡项/资金/对账/月结/成本/提成/权限/主数据链路全部真实落地，**营销 9 页收口闭合**（券/推送/核销/ROI 真实闭环）；当前工作是 **🔧 接线收口**（积分/标签、财务边角页、营销设置）与**近线 ⬜ 补点**（消息通知多渠道、排队接待、资产转移等）。
...
| P5-B22 | 2026-09-08 | 营销 9 页收口：精准推送写链路四件套（参数校验/幂等 dedupKey 60s/周频硬上限 3/违禁词四类实时校验 + 全动作审计 PUSH-SEND）+ 券核销门店三级兜底（登录门店→store-service /internal/first 内部端点→SST01 降级，FORGED 异常单也落真实门店）+ 核销链路漏斗播种器（CH-001/002/003）+ 海报推荐人接真实员工（listStaff 19 名在职，替换 mock 林晚）+ 9 页浏览器逐页真实验证零控制台报错 | `<待回填>` | DELIVERY-P5-B22 | 营销 9 页 ✅（优惠券/精准推送/券核销/营销总览/素材库/海报/直播短视频/营销ROI/渠道业绩；34→43/166=26%） |
